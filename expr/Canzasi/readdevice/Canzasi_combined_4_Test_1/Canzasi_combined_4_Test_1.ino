...
// Global Valiables
static uint8_t tx_data[ADC_CHANNELS] = {0,0,0,0};
static uint8_t tx_base[ADC_CHANNELS] = {0,10,20,30};
...
static void onI2CWrite(uint8_t *pbuf, uint8_t len){
  ...
  if((data >= REGISTER_MIN)&&(data <= REGISTER_MAX)){
    currentCh = data- 0x10;
    tx_data[currentCh] = tx_base[currentCh] + tx_cnt;  
    i2cs.setTxBuffer(&tx_data[currentCh],TX_BUFFER_SIZE);
//    if(data == REGISTER_MAX){
      tx_cnt++;
      if(tx_cnt > 9){
        tx_cnt = 0;
      }
